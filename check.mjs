...
assert.equal(isWallHost("?k=xi8k2m", "xi8k2m"), true);
...

{
  ...
  const wedding = JSON.parse(readFileSync(new URL("./data/wedding.json", import.meta.url), "utf8"));
  assert.equal(wedding.wallHost, undefined);
  const bless = wedding.blessing;
  ...
  assert.match(js, /id="wallBin"/);
  assert.match(js, /id="wallBinList"/);
  assert.match(js, /id="wallBinEmpty">清空回收站</);
  assert.match(js, /data-drop/);
  assert.match(js, /data-restore/);
  assert.match(js, /点对应的撤下，可拿掉任意一幅/);
  assert.match(js, /回收站 · 撤下后可放回/);
  assert.doesNotMatch(js, /撤下我的/);
  assert.doesNotMatch(js, /id="wallWipe"/);
  assert.doesNotMatch(js, /清空全部/);
  assert.doesNotMatch(js, /kind: "wall-wipe"/);
  ...
  assert.match(js, /kind: "wall-drop"/);
  assert.match(js, /kind: "wall-bin"/);
  assert.match(js, /kind: "wall-restore"/);
  assert.match(js, /kind: "wall-bin-empty"/);
  assert.match(js, /网上未能撤下/);
  assert.match(js, /已撤下，可在回收站放回/);
  assert.match(js, /已放回墙上/);
  assert.doesNotMatch(js, /kind: "wall-mine", by/);
  assert.match(js, /class="wall-sheet-bg"/);
  assert.match(js, /横屏书写效果更佳哦/);
  assert.match(css, /\.wall-sheet-bg[\s\S]{0,280}rgba\(139, 36, 28, 0\.6\)/);
  ...
  assert.match(html, /location\.replace\("https:\/\/sumuyang\.asia" \+ \(location\.pathname \|\| "\/"\) \+ location\.search \+ location\.hash\)/);
  const manage = readFileSync(new URL("./manage.html", import.meta.url), "utf8");
  assert.doesNotMatch(manage, /WIDD_HOST/);
  assert.match(js, /管理口令/);
  assert.match(js, /kind: "wall-auth"/);
  assert.match(manage, /js\/invite\.wx\.js/);
  assert.ok(existsSync(new URL("./js/invite.wx.js", import.meta.url)));
  const worker = readFileSync(new URL("./pages-wall/_worker.js", import.meta.url), "utf8");
  assert.match(worker, /BIN_INDEX/);
  assert.match(worker, /kind === "wall-bin"/);
  assert.match(worker, /kind === "wall-restore"/);
  assert.match(worker, /kind === "wall-bin-empty"/);
  assert.doesNotMatch(worker, /wall-wipe/);
  const readme = readFileSync(new URL("./README.md", import.meta.url), "utf8");
  assert.match(readme, /https:\/\/sumuyang\.asia\//);
  assert.match(readme, /https:\/\/sumuyang\.asia\/manage\.html/);
