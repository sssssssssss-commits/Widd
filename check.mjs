...

{
  ...
  assert.match(html, /<b>—<\/b><i>·<\/i><span>轻触开启<\/span><i>·<\/i><b>—<\/b>/);
  ...
  assert.match(css, /\.wall-actions button[\s\S]{0,360}border-radius:\s*999px/);
  assert.match(css, /\.wall-actions button[\s\S]{0,280}font-size:\s*1\.48rem/);
  assert.match(css, /WiddCao/);
  assert.match(css, /\.venue address[\s\S]{0,200}font-family:\s*"WiddCao"/);
  ...
  assert.ok(readFileSync(new URL("./assets/fonts/cao.woff2", import.meta.url)).byteLength > 1000);
