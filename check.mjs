...

{
  ...
  assert.match(css, /\.gate-tap[\s\S]{0,200}z-index:\s*7/);
  ...
  assert.match(css, /\.names small[\s\S]{0,120}font-family:\s*"WiddWall"/);
  assert.match(css, /\.names small[\s\S]{0,160}font-size:\s*1\.84rem/);
  assert.doesNotMatch(css, /\.names small[\s\S]{0,280}-webkit-text-stroke/);
