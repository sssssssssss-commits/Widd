...
签名墙：`signatureWall` 为 `true` 时，信末可手写落款。点「签字」进入横屏题字，金色笔迹，写完点「完成」即上墙。来宾点「撤下本次」只去掉自己最近一幅。新人请用下面这支**独立页面**（不要用带 `?host=` 的请柬链接，微信常会丢掉参数）：

**https://sumuyang.asia/manage.html**

打开后要输入管理口令，服务器核对通过才能逐幅撤下。撤下的签名进回收站，点「放回」可恢复；只有清空回收站才是彻底删除。光知道这个网址还撤不了。口令不要发给宾客，也不要写进请柬页面。

同一只 `rsvp.endpoint` Worker 负责收回执和签名；没配地址时签名只留在这台手机上。
...
- 「高德出发 / 腾讯地图」能跳出
- 回执能提交或能跳到问卷
- 签名墙点「签字」横屏题字，金色字完成后上墙；「撤下本次」只清自己最近一幅；管理页可逐幅撤下，回收站可放回，清空回收站才彻底删除
- 转发给自己，看标题和缩略图
